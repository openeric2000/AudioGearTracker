...
## 待改進事項 (Known Issues / TODO)

*   **架構違規**: Controller 目前直接依賴 `AppDbContext`，而不是使用 `IRepository` 或 Service。這違反了依賴反轉原則，也讓 Core 層定義的 Repository 介面沒有被實際使用。
*   **前端一致性**: 先前的 Bootstrap 移除後，部分錯誤頁面或驗證訊息樣式可能需要微調以適配 Tailwind CSS。
*   **驗證邏輯**: 確保所有關聯屬性 (如 Brand) 的空值處理 (Null Safety)。
